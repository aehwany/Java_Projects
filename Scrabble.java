...
public class Scrabble{
     ...
     */
	 ...
	 {
		// System.out.println("for loop "+ longest.length());
		 //System.out.println("the letter is " + availableLetters[i]);
		 prefix+=availableLetters[i];

		 if (myDictionary.contains(prefix)) 
		 {
			 System.out.println("The word " + "'"+ prefix+ "'" + " is in the dictionary");
			...
			// if(prefix.length>longest.length);
			 ...
			 //System.out.println("available letters are" + newavailableLetters.length);
		     String result=longestWord(newavailableLetters, prefix);
		     	//System.out.println("result is "+result+" longest is "+longest);
		     if(result.length()>longest.length())	{longest=result;}
		     prefix = prefix.replace(prefix.substring(prefix.length()-1), "");
		     //System.out.println("endloop longest is"+ longest);
